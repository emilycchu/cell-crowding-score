"""Merge the two manual label sources into one calibration set for the v2 density/Rouleaux
pipeline:
...
fold_density(), since pooling both datasets gives enough sparser examples (~40+ in Tanzania
alone) to no longer need that fold.
...
def merge_datasets(initial_rows, tanzania_rows):
    return initial_rows + tanzania_rows
...
def main():
    ...
    rows = merge_datasets(initial_rows, tanzania_rows)

    write_csv_dicts(args.out, FIELDNAMES, rows)

    print(f"initial: {len(initial_rows)} FOVs, tanzania: {len(tanzania_rows)} FOVs, merged: {len(rows)} FOVs")
    if len(rows) != 337:
        print(f"warning: expected 337 merged FOVs, got {len(rows)}")
